...
export default () => {
	it("can create and update a user alias", async () => {
		...
		await program.methods
			.createUserAlias("Potatogirl")
			.accounts({ user: user.publicKey, userAlias: userAliasPDA })
			.rpc();
		assert.equal(
			(await program.account.userAlias.fetch(userAliasPDA)).alias,
			"Potatogirl"
		);

		await program.methods
			.updateUserAlias("Sasha")
			.accounts({ user: user.publicKey, userAlias: userAliasPDA })
			.rpc();
		assert.equal(
			(await program.account.userAlias.fetch(userAliasPDA)).alias,
	...
	it("can delete a user alias", async () => {
		...
		await program.methods
			...
			.rpc();
		assert.ok(
			(await program.account.userAlias.fetchNullable(userAliasPDA)) === null
		);
	});
};
