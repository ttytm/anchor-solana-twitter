...
const dmRecipient = new PublicKey(
	"7aCWNQmgu5oi4W9kQBRRiuBkUMqCuj5xTA1DsT7vz8qa"
);
...
const sendDm = async (user: any, recipient: PublicKey, content: string) => {
	...
	await program.methods
		...
		.signers(
			user instanceof (anchor.Wallet as any) ? [dmKeypair] : [user, dmKeypair]
		)
		.rpc();
...
export default () => {
	it("can send and update dms", async () => {
		...
		assert.equal(
			(await program.account.dm.fetch(dm.publicKey)).recipient.toBase58(),
			dmRecipient.toBase58()
		);
	...
	it("can fetch and filter dms", async () => {
		...
		assert.ok(
			allDms.every(
				(dm) => dm.account.user.toBase58() === user.publicKey.toBase58()
			)
		);
		...
		assert.ok(
			userDms.every(
				(dm) => dm.account.recipient.toBase58() == dmRecipient.toBase58()
			)
		);
	});
};
