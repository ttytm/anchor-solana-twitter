...
export default () => {
	it("can vote and update votings", async () => {
		...
		const otherUsersVoting = await vote(otherUser, tweet.publicKey, {
			dislike: {},
		});
		assert.equal(
			otherUsersVoting.account.tweet.toString(),
			tweet.publicKey.toString()
		);
	...
	it("can derive tweets from votings", async () => {
		...
		assert.ok(
			userVotings.every(
				(voting) => voting.account.user.toBase58() === user.publicKey.toBase58()
			)
		);
		...
		const likedTweets = await Promise.all(
			userVotings
				.filter(
					(voting) => Object.keys(voting.account.result).toString() == "like"
				)
